...
//!
//! [`skip_while`](InputRef::skip_while) answers without reaching the general scanner when the
//! answer is already sitting at the front of the token stream: the head is there and the
//! predicate rejects it, so the scan would skip nothing — no scan scope, no frontier clone, no
//! take-test-put-back of the stopping token, no commit of a position that did not move.
//!
...
//! agree across all twelve. That last column is the one a naive fast path fails: probing the head
//! and then handing the same token to the scanner asks a stateful `FnMut` about it twice, and a
//! predicate that answers differently the second time would skip a token the general path keeps
//! or keep one it skips.
...
#[test]
fn skip_while_fast_path_is_invisible_to_residency() {
