...
//! | `resource_trip` (a budget was exceeded) | `Input` (borrowed through `InputRef`) | **monotone session fact** | **nothing** — set-once, and there is no writer that lowers it. A rollback rewinds *input progress*; it cannot un-exceed a budget, so restoring this cell would erase a stop that has already been decided |
  ...
  Cmpl: Completeness,
{
  let Input {
    ...
    //   claiming a stop it has already been given. Set-once by `raise_level`'s trip arm, read by
    //   the three recovery combinators; no lowering writer exists. Not in `Checkpoint`.
    resource_trip: _,
  ...
  let InputRef {
    ...
    recursion: _,
    // — MONOTONE SESSION FACT (borrowed): latched by `raise_level`'s trip arm, never lowered and
    //   never restored. Same class as the `Input` field above it.
    resource_trip: _,
